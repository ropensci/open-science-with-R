...
A database holds data in a structured way. This structure can vary quite a bit, but the salient point is that any database you have locally can also go online with a few bells and whistles to allow a user to interact with the database. A database available over the internet is stored on a computer somewhere, just as when it is stored on your own - except a few tricks allow you to query that database "over the wire".

## Why use online data (or, the power of online data)
